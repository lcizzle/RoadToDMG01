...
module ssram_led_01_top
    ...
    // Infer SSRAM
    // 540 words x 32 bits = 17 280 bits
    (* ram_style = "ssram" *)
    reg [31:0]              smem [0:539];

    // Initialize FSM: write addr + smem[addr] on each clock
    reg [9:0]               addr_init;
    reg                     init_done;
    always @(posedge clk_in, posedge rst_btn) begin
        if (rst_btn) begin
            ...
            smem[addr_init] <= addr_init;
            addr_init <= addr_init + 10'd1;

            if (addr_init == 10'd539) begin
                init_done <= 1'b1;
            end
        end
    end
